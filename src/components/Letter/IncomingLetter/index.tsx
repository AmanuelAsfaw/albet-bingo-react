...
import ShareLetterComponent from "./components/ShareLetter/ShareLetter.component";
...
const IncomingLetterComponent: FC<LetterPropType> = ({
  fetchLetter,
  letters,
  tab,
}) => {
  ...
  useEffect(() => {
    fetchLetter();
  }, [fetchLetter, tab, fetchAllProjects]);

  useEffect(() => {
    if (letters.payload.length > 0) {
      setIncomingData(
        parsePayloadData(letters.payload).map((item: any, index: any) => ({
          key: index,
          ...item,
        }))
      );
  ...
  const column: ColumnsType<Letter> = [
    ...
    },
    {
      ...
      key: "from",
      width: "200px",
      render: (value, record) => record.from,
    },
    {
      title: "Document",
      key: "document",
