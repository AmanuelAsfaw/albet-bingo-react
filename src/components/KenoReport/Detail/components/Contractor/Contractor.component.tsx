...
import { ContractorPropType, editData } from "../../utils/Detail.util";
...
const ContractorComponent: FC<ContractorPropType> = ({
  contractors,
  fetchContractors,
  fetchProject,
  project,
  ...
  useEffect(() => {
    let found = contractors.payload.find((e) => e.name === contractor);
    if (found) form.setFieldsValue(found);
  }, [contractors, contractor, form]);

  useEffect(() => {
    if (project.payload) form.setFieldsValue(project.payload?.contractor);
  }, [project, form]);
          ...
          <Form layout="vertical" form={form} onFinish={Submit}>
            <div className="row">
              <div className="col-md-4">
                ...
                >
                  <AutoComplete
                    onChange={(e) => setContractor(e)}
                    placeholder="name"
                    options={contractors.payload.map((e, index) => ({
                      name: e.name,
                      value: e.name,
                      key: index,
                    }))}
...
const mapStateToProps = (state: any) => ({
  contractors: state.contractor.fetchAll,
...
const mapDispatchToProps = (dispatch: any) => ({
  fetchContractors: (action: any) => dispatch(fetchAllContractors(action)),
