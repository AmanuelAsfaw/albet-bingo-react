import { Button, Checkbox, DatePicker, Form, Input, Modal, Statistic, Upload } from "antd";
...
const CommentComponent: FC<CommentPropType> = ({
  project,
  users,
  kling_material_approval,
  fetchAllKlingMaterialApproval,
}) => {
  console.log({ kling_material_approval });
  ...
  const Submit = (value: any) => {
    setLoading(true);
    const data = {
      id: kling_material_approval.kling_ma_comment?.id,
      kling_material_approval_id: kling_material_approval.id,
      ...comments[0],
      date: moment().format("YYYY-MM-DD"),
      commented_by: getUserData().full_name,
      reviewed_by: getUserData().full_name,
      type: "comment",
    };
    ...
    updateCommentData(data)
      .then(() => {
        ...
        OpenNotification(NotificationType.SUCCESS, "Comments saved!", "");
      })
      .catch((error) => {
        setLoading(false);
        ...
        <Form layout="vertical" onFinish={Submit} form={form}>
          ...
          <div className="row">
            ...
            <div className="col-md-3">
              ...
              >
                <Input value={kling_material_approval.deliver_order_no} disabled />
              </Form.Item>
            </div>
          </div>
          ...
          <div className="row">
            ...
            <div className="ml-3">
              <Checkbox checked={is_approval}>Approval</Checkbox>
              <Checkbox checked={is_selection}>Selection</Checkbox>
              <Checkbox checked={is_shop}>Shop Drawing Approval</Checkbox>
              <Checkbox checked={is_others}>Others</Checkbox>
            </div>
          </div>
