...
import { KlingMaterialApprovalRemarkPropType, parseData, sendRemark, updateSeen } from "../../util/KlingMaterialApproval.util";
import { fetchAllUser } from "../../../../../../redux/User/User.action";
...
import DocumentViewerComponent from "../../../../../common/DocumentViewer/DocumentViewer.component";
import { OpenNotification } from "../../../../../common/Notification/Notification.component";
import { DownloadFile } from "../../../Submittal/util/Submittal.util";
import { useParams } from "react-router-dom";

const KlingMaterialApprovalRemark: FC<KlingMaterialApprovalRemarkPropType> = ({
  remarkData,
  users,
  ...
  const handleOk = () => {
    ...
    if (user) {
      sendData = {
        id: user?.["user_kma"].id,
        kling_material_approval_id: remarkData.id,
        user_id: id,
      };
    } else {
      sendData = {
  ...
  const Submit = (value: any) => {
    ...
    formData.append("kling_material_approval_id", `${remarkData.id}`);
    ...
    sendRemark(formData)
      .then(() => {
        ...
        setLoading(false);
        fetchKlingMaterialApproval({project_id: params?.id});
        OpenNotification(NotificationType.SUCCESS, "Remark added", "");
      })
      .catch((error) => {
        setLoading(false);
        ErrorHandler(error).map((e: any) =>
  ...
  const renderRemark = () => {
    ...
    remarkData.kling_material_approval_remarks
      .sort((a: any, b: any) =>
...
const mapDispatchToProps = (dispatch: any) => ({
  fetchKlingMaterialApproval: (action: any) => dispatch(fetchAllKlingMaterialApproval(action)),
  fetchAllUser: () => dispatch(fetchAllUser()),
});

export default connect(mapStateToProps, mapDispatchToProps)(KlingMaterialApprovalRemark);
